...
class SimulatedThrusters(object):
    def __init__(self, name, topic_input, topic_feedback, topic_forces, thruster_limit, **kwargs):
        ...
        # data arrays
        self.v_requested = np.zeros((6, tm.LPF_WINDOW))
        ...
        self.model_delay  = np.clip(self.model_delay, 0, tm.LPF_WINDOW-1)
    ...
    def handle_commands(self, data):
        ...
        self.v_predicted = tm.thruster_model_predict(self.v_requested, offset=self.model_delay, limit=self.throttle_limit)

        if self.limit_rate is True:
            # new thrusters filter
            self.v_predicted = tm.thruster_rate_limiter(
                self.v_predicted, self.v_last, rising_limit=self.rising_limit, falling_limit=self.falling_limit
            )

        self.i_predicted = tm.thruster_model_current(self.v_predicted)
        ...
        self.forces_predicted = tm.thruster_model_forces( self.v_predicted, self.i_predicted )
